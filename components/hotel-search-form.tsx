...
const HotelSearchForm: React.FC<HotelSearchFormProps> = ({ setLoading, setResults }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      checkOut: params.get("checkOut") ? new Date(params.get("checkOut")!) : new Date(Date.now() + 24 * 60 * 60 * 1000),
      rooms: [],
      currency: params.get("currency") || "USD",
      nationality: params.get("nationality") || "NG",
      destinationCode: params.get("destination") || "",
      totalRooms: params.get("totalRooms") ? parseInt(params.get("totalRooms") || "1") : 1,
    },
  });
        ...
        if (room.id === roomId) {
          if (field === "children") {
            ...
            if (newChildren > currentChildren) {
              const newAges = [...room.childrenAges, ...Array(newChildren - currentChildren).fill(0)];
  ...
  const renderFullForm = (isMobile = false) => (
    <form onSubmit={handleSubmit(onSubmit)} className="w-full">
      ...
      >
        ...
        <div className={`flex gap-4 ${isMobile ? 'flex-col' : 'flex-row'} w-full`}>
          {/* City Search with Dropdown */}
          <div className={`${isMobile ? 'w-full' : 'flex-1'} w-full`}>
            <label className="block text-[#808080] mb-1 text-sm">Destination</label>
            ...
            {errors.destination && (
              <p className="text-red-500 text-xs mt-1 w-full">{errors.destination.message}</p>
            )}
          </div>

          {/* Date Selection */}
          <div className={`flex gap-4 ${isMobile ? 'w-full' : 'flex-1'} w-full`}>
            <div className="flex-1 w-full">
              <label className="block text-[#808080] mb-1 text-sm w-full">Check in</label>
              ...
              {errors.checkIn && (
                <p className="text-red-500 text-xs mt-1 w-full">Check-in date is required</p>
              )}
            </div>

            <div className="flex-1 w-full">
              <label className="block text-[#808080] mb-1 text-sm w-full">Check out</label>
              ...
              {errors.checkOut && (
                <p className="text-red-500 text-xs mt-1 w-full">Check-out date is required</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col w-full">
          <div className="flex items-center justify-between mb-2 w-full">
            <label className="block text-[#808080] text-sm">Rooms & Guests</label>
            {rooms.length < 3 && (
              <Button
                type="button"
                variant="ghost"
          ...
          <div className="space-y-3 w-full">
            {rooms.map((room, index) => (
              <div key={room.id} className="border rounded-lg p-3 bg-gray-50 w-full">
                ...
                <div className="flex gap-4 w-full">
                  <div className="flex-1">
                    <Counter
                      label="Adults"
                      value={room.adults}
                      onIncrement={() => updateRoom(room.id, "adults", room.adults + 1)}
                      onDecrement={() => updateRoom(room.id, "adults", Math.max(1, room.adults - 1))}
                      min={1}
                    />
                  </div>
                  <div className="flex-1">
                    <Counter
                      label="Children"
                      value={room.children}
                      onIncrement={() => updateRoom(room.id, "children", room.children + 1)}
                      onDecrement={() => updateRoom(room.id, "children", room.children - 1)}
                    />
                  </div>
                </div>
                ...
                {room.children > 0 && (
                  <div className="mt-3 space-y-2 w-full">
                    <label className="block text-[#808080] text-xs">Children Ages</label>
                    <div className="flex flex-wrap gap-2 w-full">
                      {room.childrenAges.map((age, childIndex) => (
                        <Select
                          key={childIndex}
                          value={age.toString()}
                          onValueChange={(value) => updateChildAge(room.id, childIndex, parseInt(value))}
                        >
          ...
          {errors.rooms && (
            <p className="text-red-500 text-xs mt-1 w-full">Room configuration is required</p>
          )}
        </div>
        ...
        <div className={`flex gap-4 ${isMobile ? 'flex-col' : 'flex-row justify-between items-end'} w-full`}>
          {/* Currency Selection */}
          <div className={isMobile ? 'w-full' : 'w-48'}>
            <label className="block text-[#808080] mb-1 text-sm">Currency</label>
            <Select 
              value={currencyValue} 
          ...
          <div className={isMobile ? 'w-full' : 'w-48'}>
            <label className="block text-[#808080] mb-1 text-sm">Nationality</label>
            <Select
          ...
          <Button 
            type="submit"
            className={`h-10 bg-blue-600 hover:bg-blue-700 ${isMobile ? 'w-full mt-2' : 'w-48'}`}
          >
            Search Hotels
          </Button>
        </div>
        ...
        >
          <div className="flex items-center gap-3 w-full">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm truncate">{destinationValue}</p>
              <p className="text-xs text-gray-500 truncate">
                {formatDate(checkInValue)} - {formatDate(checkOutValue)} • {totalGuests}{" "}
                guest{totalGuests !== 1 && "s"} • {rooms.length} room{rooms.length !== 1 && "s"}
              </p>
            </div>
          </div>
        </Button>
